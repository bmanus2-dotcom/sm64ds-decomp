
extern void func_ov019_02112268(char *c);
extern void _ZN9Animation7AdvanceEv(char *c);
extern void _ZN12CylinderClsn5ClearEv(char *c);
extern void _ZN12CylinderClsn6UpdateEv(char *c);
extern void p__sinit_ov031_02111434(char *c);
int func_ov019_02112394(char *c) {
    func_ov019_02112268(c);
    _ZN9Animation7AdvanceEv(c+0x124);
    _ZN9Animation7AdvanceEv(c+0x138);
    _ZN12CylinderClsn5ClearEv(c+0x174);
    _ZN12CylinderClsn6UpdateEv(c+0x174);
    p__sinit_ov031_02111434(c);
    return 1;
}
